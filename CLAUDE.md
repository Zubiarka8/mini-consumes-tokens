...
| Only know what the code *does*, not words of its name (`load settings from disk`)? | `hybrid_search` — `search_symbols` fused with local-embedding similarity by weighted Reciprocal Rank Fusion; `alpha` 0 = lexical only … 1 = semantic only (default 0.5), `top_k` 10 (max 100), `offset`, `snippet_lines`. Needs a `--features semantic` build of `mct-mcp-server` (model downloaded on first use into `.mct-index/models`); otherwise it returns the lexical ranking and says so on its first line |
