...
2. Extend `crates/mct-lang-md` (currently "Phase 1": ATX headings only, zero relations — see its module doc comment) to index the PKM conventions used by that vault — `[[WikiLinks]]` and `#tags` — into the existing symbol/relation model, with **no changes to the shared `mct-core` schema**.
...
docs/
  ...
  02-crates/
    ...
    parsers/
      ...
      mct-lang-php.md      # exception case only, per Decision 6
  03-performance/
    limits-spec.md
  04-security/
    advisories/
...
- Cross-linked via `[[WikiLink]]` to related notes (e.g. `sec-001-php-stack-overflow.md` links `[[mct-lang-php]]` and `[[overview]]`).
...
`crates/mct-lang-md/tests/parse.rs`:
...
`crates/mct-lang-md/tests/index_integration.rs`: extend or add a fixture exercising a wikilink + tag through the real SQLite index (`find_references`-style query), matching the existing fixture style in `tests/fixtures/docs-project/`.

## Validation

- `cargo check --workspace`
- `cargo test -p mct-lang-md`
- `cargo clippy --workspace --all-targets --all-features -- -D warnings`
- Manual check: `docs/02-crates/parsers/` contains exactly `overview.md` and `mct-lang-php.md`, nothing else.
