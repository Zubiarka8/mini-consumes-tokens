...
- No fix for whole-note resolution depending on the "a note's own heading is
  ...
  vaults, but touches `mct-core` and reverses Phase 2's approved
...
`crates/mct-lang-md/tests/parse.rs`: anchor splits into two relations (alias
...
`crates/mct-lang-md/tests/index_integration.rs` + `fixtures/docs-project/architecture.md`:
...
(matches pre-existing `mct-index` behavior — a relation is stored regardless
...
- Whole-note resolution independent of the heading-naming convention (would
  need a `SymbolKind::Document`-style change to `mct-core`) — explicitly
