# ccm-lang-md Phase 2 (WikiLink/Tag Relations) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Extend `crates/ccm-lang-md` to index `[[WikiLink]]` and `#tag` occurrences in heading/paragraph text as `RelationKind::References` relations, with zero changes to `ccm-core`'s schema.
...
- Production code in `crates/ccm-lang-md/src/` must never `unwrap()`/`panic!`/`expect()` on repo-input content (project-wide invariant).
...
**Files:**
- Modify: `crates/ccm-lang-md/src/lib.rs`
...
```bash
git add crates/ccm-lang-md/src/lib.rs
git commit -m "refactor(ccm-lang-md): thread parent_id through the heading walker"
```
...
**Files:**
- Modify: `crates/ccm-lang-md/src/lib.rs`
- Modify: `crates/ccm-lang-md/tests/parse.rs`
...
In `crates/ccm-lang-md/tests/parse.rs`, change the import line:
...
Run: `cargo test -p ccm-lang-md wikilink_in_a_paragraph_emits_a_references_relation`
...
```bash
git add crates/ccm-lang-md/src/lib.rs crates/ccm-lang-md/tests/parse.rs
git commit -m "feat(ccm-lang-md): extract [[WikiLinks]] as References relations"
```
...
Add to `crates/ccm-lang-md/tests/parse.rs`:
...
Run: `cargo test -p ccm-lang-md hashtag_in_a_paragraph_emits_a_tag_prefixed_relation`
Expected: FAIL.
...
```bash
git add crates/ccm-lang-md/src/lib.rs crates/ccm-lang-md/tests/parse.rs
git commit -m "feat(ccm-lang-md): extract #tags as tag:-prefixed References relations"
```
...
**Files:**
- Modify: `crates/ccm-lang-md/src/lib.rs` (module doc comment only)
- Modify: `crates/ccm-lang-md/tests/parse.rs`
...
Run: `cargo test -p ccm-lang-md a_wikilink_before_any_heading_is_not_indexed`
...
Replace the whole doc comment block at the top of `crates/ccm-lang-md/src/lib.rs`:
```rust
...
```bash
git add crates/ccm-lang-md/src/lib.rs crates/ccm-lang-md/tests/parse.rs
git commit -m "test(ccm-lang-md): lock in orphan-link skip; update module doc comment for Phase 2"
```
...
**Files:**
- Modify: `crates/ccm-lang-md/tests/fixtures/docs-project/architecture.md`
- Modify: `crates/ccm-lang-md/tests/index_integration.rs`

**Interfaces:**
- Consumes: `Index::find_references(&self, symbol: &str) -> Result<Vec<RelationHit>>` (existing `ccm-index` API; `RelationHit` has `kind: String`, `from_symbol: String`, `to_name: String`).

- [ ] **Step 1: Add a WikiLink and a tag to the fixture**

Replace the full contents of `crates/ccm-lang-md/tests/fixtures/docs-project/architecture.md`:
```markdown
# Architecture
...
Add to `crates/ccm-lang-md/tests/index_integration.rs`:
...
```bash
git add crates/ccm-lang-md/tests/fixtures/docs-project/architecture.md crates/ccm-lang-md/tests/index_integration.rs
git commit -m "test(ccm-lang-md): verify WikiLink/tag relations resolve through the real SQLite index"
```
...
with:
```
| Markdown | Implemented (Phase 2 — ATX headings as `Element` symbols; `[[WikiLinks]]`/`#tags` in heading/paragraph text as `References` relations, tags prefixed `tag:`; anchors indexed verbatim, no anchor-aware resolution; lists, tables, blockquotes, and code spans not scanned) | `crates/ccm-lang-md` |
```
...
with:
```
  ccm-lang-md        LanguageParser impl for Markdown (tree-sitter-md) — ATX headings as nested Element symbols via the grammar's own section nesting; Phase 2 adds `[[WikiLink]]`/`#tag` extraction from heading/paragraph text as `References` relations
```
...
Expected: either `ccm-lang-php.md` / `overview.md` (nothing else), or the "not yet created" message if the docs-vault plan hasn't run yet — both are fine, this task doesn't depend on it.

- [ ] **Step 8: Commit**

```bash
git add README.md
git commit -m "docs(ccm-lang-md): update README for Phase 2 WikiLink/tag relation support"
```
