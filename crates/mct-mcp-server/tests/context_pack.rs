...
async fn packs_definition_doc_comment_and_every_related_symbol_once() {
    ...
    assert!(
        text.contains("src/orders.rs:L35-L49 [rust] function place_order"),
        "{text}"
    );
    assert!(
        text.contains("31| /// Places `order`: checks it is well formed"),
        "{text}"
    );
    ...
    assert!(
        text.contains("callee validate_order  src/orders.rs:L52-L62 function"),
        "{text}"
    );
    assert!(
    ...
    assert!(
        text.contains("caller,test test_place_order_stores_the_discounted_total"),
        "{text}"
    );
    assert!(
        text.contains("caller place_orders  src/orders.rs:L82-L91 function"),
        "{text}"
    );
...
async fn one_pack_cuts_tokens_by_at_least_60_percent_versus_manual_aggregation() {
    ...
    let packed = exchange(
        ...
        "build_context_pack",
        &args,
        &pack(&server, args.clone()).await,
    );

    let (manual, tools_only, packed) = (
        approx_tokens(&manual),
        approx_tokens(&tools_only),
        approx_tokens(&packed),
    );
...
async fn toon_renders_the_related_symbols_as_one_table() {
    ...
    let text = pack(
        &server,
        json!({ "symbol": "place_order", "format": "toon" }),
    )
    .await;
    assert!(
...
async fn depth_two_adds_callees_of_callees_tagged_with_their_hop() {
    ...
    assert!(
        deep.starts_with("Context pack for `place_order` (depth 2)"),
        "{deep}"
    );
}
...
async fn an_ambiguous_name_packs_each_definition_and_path_narrows_it() {
    ...
    assert!(
        both.contains("src/inventory.rs:L13-L15 [rust] method new"),
        "{both}"
    );
    assert!(
        both.contains("src/store.rs:L22-L24 [rust] method new"),
        "{both}"
    );
    ...
    assert!(
        !one.contains("src/inventory.rs:L13-L15 [rust] method new"),
        "{one}"
    );
}
...
async fn source_lines_caps_the_body_and_says_how_much_is_hidden() {
    ...
    let text = pack(
        &server,
        json!({ "symbol": "place_order", "source_lines": 1 }),
    )
    .await;
    assert!(text.contains("35| pub fn place_order("), "{text}");
    assert!(!text.contains("36|"), "{text}");
    assert!(
        text.contains("… 14 more line(s) — raise `source_lines`"),
        "{text}"
    );
}
...
async fn an_unknown_symbol_says_so_and_an_empty_name_is_rejected() {
    ...
    assert!(
        text.starts_with("No symbol named `no_such_symbol`"),
        "{text}"
    );
