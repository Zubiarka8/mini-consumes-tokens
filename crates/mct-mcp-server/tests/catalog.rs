...
async fn the_full_catalogs_serialized_footprint_is_well_under_the_pre_ttc_size() {
    ...
    // at 2024 bytes (~68.5% smaller) and sits at ~2690 bytes across today's
    // 14 tools after `explain_symbol` was added. 2900 is a regression guard
    // with headroom for a couple more tools, not a precise assertion — bump
    // it (with a note here) rather than loosen it silently if a future tool
    // needs the room.
    assert!(
