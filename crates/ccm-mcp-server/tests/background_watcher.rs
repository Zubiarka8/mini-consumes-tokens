//! Exercises `ccm_mcp_server::background::spawn_watcher` directly against a
...
use ccm_mcp_server::background;
use tokio::sync::Mutex;

/// A fresh temp directory, matching the hand-rolled convention already used
/// by `crates/ccm-index/tests/reindex.rs` (no `tempfile` dependency needed
...
fn tempdir() -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!("ccm-mcp-server-test-{}", uuid_like()));
...
async fn a_change_under_an_excluded_path_does_not_trigger_a_reindex() {
    ...
    // node_modules/** is excluded by default (crates/ccm-index/src/exclude.rs)
    // — a change only here must never trigger a reindex, or writing the
    // index's own `.ccm-index/` database back to itself would loop forever.
