...
7. **Keep noise out (optional):** `mct-cli --root . ignore-init` writes a starter `.mctignore` — edit it to exclude things like `docs/` or `*.md` from the index, then `reindex --force`. Pass `--import-gitignore` to opt into excluding everything your project's own `.gitignore` already excludes, right from creation. And `mct-cli --root . gitignore-init` keeps the generated database itself out of git.
...
| `ignore-init --import-gitignore` | Same, but the generated file already has `@import-gitignore` active: everything the project's own `.gitignore` excludes is excluded from indexing too (its `!negation` lines are skipped, same simplification `.mctignore` itself has). Only takes effect when the file is actually created — an existing `.mctignore` is left untouched either way; add the line yourself to opt in later. |
| `gitignore-init` | Adds `.mct-index/` to the project's `.gitignore` (creating it if needed), so the generated symbol database never ends up committed — it's fully derived from source and regenerated by `reindex`. Additive and idempotent: leaves everything else in the file untouched, and does nothing if it's already covered. |
