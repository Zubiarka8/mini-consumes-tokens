...
| `dead-code` | Writes a CSV report of dead-code candidates (heuristic: symbols with zero indexed references) to `.mct-index/dead-code-report.csv` — one row per candidate with its file, function name, kind, language, and start/end line. |
| `dead-code --path <path> --language <lang> --output <file>` | Same, narrowed to a file/directory/crate and/or language, and written to a custom path instead of the default. |
