...
Methodology (`crates/mct-cli/examples/token_benchmark.rs`, run via `cargo run -p mct-cli --example token_benchmark`):
...
## Java (`crates/mct-lang-java/tests/fixtures/billing-app`)
...
## C# (`crates/mct-lang-csharp/tests/fixtures/billing-app`)
...
## JavaScript/TypeScript (`crates/mct-lang-js-ts/tests/fixtures/webapp`)
...
## C++ (`crates/mct-lang-cpp/tests/fixtures/billing-app`)
...
5-file fixture (`Invoice.h`/`.cpp`, `Logger.h`/`.cpp`, `Main.cpp`) — one more file matches per grep query than Java/C#/JS-TS because the header/source split means both the declaration and the definition are real, separate matches, which is exactly the case this crate is built to handle (see `mct-lang-cpp/src/lib.rs`'s module doc). The reduction holds despite that extra file.

## Go (`crates/mct-lang-go/tests/fixtures/billing-app`)
...
## Rust (`crates/mct-lang-rust/tests/fixtures/billing-app`)
...
## Python (`crates/mct-lang-python/tests/fixtures/billing-app`)
...
## Lua (`crates/mct-lang-lua/tests/fixtures/inventory-app`)

Reused the existing `inventory-app` fixture and its proven query terms (same ones `index_integration.rs` already asserts on). `mct-lang-lua` is deliberately **not** a `mct-cli` dependency (see its `Cargo.toml` description — it's an architecture-validation exercise outside the project's original language scope, not wired into the production `mct-cli`/`mct-mcp-server` registries), so this row was produced by a separate, self-contained harness — `crates/mct-lang-lua/examples/token_benchmark_lua.rs` — that duplicates the exact same `mcp_chars`/`grep_then_read_chars` measurement code rather than adding Lua to `mct-cli`. Run via `cargo run -p mct-lang-lua --example token_benchmark_lua` (named `_lua` to avoid an example-binary filename collision with `mct-cli`'s own `token_benchmark` example).
...
## HTML (`crates/mct-lang-html/tests/fixtures/site`)
...
**Outlier, called out explicitly, not smoothed over**: the "what calls this function" row is a degenerate 100.0%, and it is *not* a genuine efficiency win. `mct-lang-html` (like `mct-lang-css` and `mct-lang-xaml` below) never emits a `RelationKind::Calls` relation — HTML has no function-call semantics — so `find_callers` returns 0 hits (0 chars) for *any* term, while grep+read still has to scan and read whatever files happen to contain the literal text. The 100% figure measures "MCP correctly reports zero callers because there are none to report" against "grep still had to look", not a real navigation win. Real, structural, and expected — not a bug or a cherry-picked term.

## CSS (`crates/mct-lang-css/tests/fixtures/theme`)
...
**Outlier, called out explicitly**: same degenerate 100.0% as HTML above and for the same reason — `mct-lang-css` only ever emits `Imports` relations, never `Calls`, so `find_callers` is structurally always empty for CSS. **Also notable, on the low side**: `#header` (87.8%) and `extra.css` (82.8%) both fall below the 88–98% band the first 5 languages established — a real measured result, not an error. The fixture is intentionally tiny (2 files, ~15 lines total), so the fixed per-line overhead of the MCP-formatted output (path:line:col + kind + name) is proportionally larger against a very small baseline; the same effect shows up in XML below, more severely.

## XML (`crates/mct-lang-xml/tests/fixtures/services-config`)

The pre-existing `tests/fixtures/config` fixture is a single 8-line file — too trivial for a meaningful multi-file comparison against the other benchmarked languages, so a new 3-file fixture (`api.xml`/`worker.xml`/`logging.xml`, same nested-`id`/`name`/`Name` shape as `config`) was added this session for benchmarking; `config` itself is untouched and still used by `mct-lang-xml`'s own integration tests.
...
- **Both relation queries are structurally 100.0% for XML, always, regardless of fixture or term.** `mct-lang-xml`'s own module doc states it plainly: "No relations are ever emitted — there is nothing dialect-generic to link to" (generic XML has no universal cross-file-reference convention, unlike XAML). So `find_callers` *and* `find_references` both return 0 hits for every term in every XML fixture — this isn't a fixture limitation fixable by making the fixture bigger or more realistic, it's the parser's deliberate scope boundary. Two of the three canonical queries are not meaningfully answerable by `mct-lang-xml` today.
...
## XAML (`crates/mct-lang-xaml/tests/fixtures/app`)
...
**Outlier, called out explicitly**: the same structural 100.0% as HTML/CSS above — `mct-lang-xaml` only emits `References` relations (event-handler-attribute → C# method), never `Calls`, so `find_callers` is always empty. Unlike XML, XAML's `find_references` row (90.6%) is real and meaningful — it resolves a genuine cross-*language* relation (a XAML `Click` attribute back to its C# method body) — so only the "callers" query is degenerate here, not two of three.

## Bash (`crates/mct-lang-bash/tests/fixtures/deploy-scripts`)
...
## PowerShell (`crates/mct-lang-powershell/tests/fixtures/deploy-scripts`)
...
## PHP (`crates/mct-lang-php/tests/fixtures/billing-app`)
...
- **`mct-lang-css`, `mct-lang-html`, and `mct-lang-xaml` never emit a `Calls` relation** (they're declarative/markup languages without function-call semantics), so their "what calls this function" query is *always* a degenerate 100.0% reduction (0 MCP hits vs. a non-zero grep+read baseline) — a real, structural result, not a genuine navigation win, and not comparable to the same query on Java/Rust/Bash/etc.
- **`mct-lang-xml` never emits *any* relation** (by explicit design — see its module doc), so *both* "what calls this function" and "who uses this symbol" are always 100.0% for XML, on top of its `find_symbol` reduction (72.0%) being the lowest of any query measured this session, from small-fixture fixed-overhead. XML is the weakest-covered language by this benchmark's own three-query methodology; only 1 of 3 canonical queries produces a meaningful number for it today.
